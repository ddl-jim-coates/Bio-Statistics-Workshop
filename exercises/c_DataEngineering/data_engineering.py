...
clean_filename = (
    "clean_trial_data.csv"  # Input: cleaned clinical trial data (compatible naming)
)
features_filename = (
    "transformed_trial_data.csv"  # Output: preprocessed clinical features
)
...
domino_project_owner = os.environ.get(
    "DOMINO_PROJECT_OWNER", os.environ.get("DOMINO_USER_NAME", "default-owner")
)
...
domino_dataset_dir = (
    f"{domino_working_dir.replace('code', 'data')}/{domino_project_name}"
)
...
def add_derived_features(df):
    ...
    Derived Features:
        - bmi_biomarker_ratio: BMI relative to baseline biomarker levels
          (identifies metabolic imbalance patterns)
        - clinical_risk_score: Combined comorbidity and genetic risk scores
        - adherence_risk_balance: Treatment adherence adjusted for comorbidity burden
    ...
    df["bmi_biomarker_ratio"] = df["BMI"] / (df["BaselineBiomarker"] + 1e-6)
    ...
    df["clinical_risk_score"] = (df["ComorbidityScore"] + df["GeneticRisk"]) / 2
    ...
    df["adherence_risk_balance"] = df["Adherence"] - df["ComorbidityScore"]
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="Preprocessing Pipeline") as run:
        ...
        # AdverseEvent: 0 = no adverse event, 1 = serious adverse event occurred
        labels_df = full_cleaned_df["AdverseEvent"]
        features_df = full_cleaned_df.drop(columns=["AdverseEvent"], errors="ignore")
        ...
        transformed_features_df = pd.DataFrame(
            transformed_features_array, columns=feature_names, index=features_df.index
        )

        # Add labels back to the transformed features
        transformed_features_df["AdverseEvent"] = labels_df
